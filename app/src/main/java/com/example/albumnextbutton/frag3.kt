...
import com.example.albumnextbutton.databinding.FragmentFrag3Binding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        button?.setOnClickListener {
            if(button.text == "Disconnect"){
                button.text = "Connect"
                requireActivity().stopService(Intent(context, AIDLService::class.java))

            }
            else{
                button.text = "Disconnect"

                requireActivity().startForegroundService(Intent(context, AIDLService::class.java))
